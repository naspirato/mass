        ...
        for idx, viz_data in enumerate(visualization_data):
            try:
                ...
                if series is None or series.empty:
                    print(f"    ⚠ Skipping visualization for group {idx}: series is empty")
                    continue
                ...
                if len(series) > 0:
                    fig.add_trace(go.Scatter(
                        x=series.index,
                        y=series.values,
                        mode='lines+markers' if len(series) > 1 else 'markers',  # Use only markers for single point
                        name='Metric Value',
                        line=dict(color='blue', width=2) if len(series) > 1 else None,
                        marker=dict(size=8 if len(series) == 1 else 4)  # Larger marker for single point
                    ))
